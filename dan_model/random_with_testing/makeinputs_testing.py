...
def pickle_instances():
    ...
    for entry in l:
        if entry[1] == SUBORD:
            for i in xrange(3):
                name = entry[0] + str(i)
                individuals[name] = entry[2] + ["color" + str(randint(0,19)), \
                                                "pose" + str(randint(0,19)), \
                                                "size" + str(randint(0,19)) ]                                           
    pickle.dump(individuals,outfile)
    outfile.close()

    test_items = dict()
    outfile = ("testitems.pkl",w)
    test_names = ['dalmatian0','dalmatian1','poodle0','pug0','tabby0','manx0','flounder0','fire-truck0','motorboat0']
    for n in test_names:
        test_items[n + "T"] = individuals[n][:-3] + ["color" + str(randint(0,19)), \
                                                     "pose" + str(randint(0,19)), \
                                                     "size" + str(randint(0,19)) ]
...
for (word, level, freq) in l.items():
    if level == SUPERORD:
        supers += 1
    elif level == BASIC:
        basics += 1
...
FT_VALS = BASIC_FREQ *basics + SUPERRO_FREQ * supers + SUBO_FREQ * subs 
print FT_VALS
